...
export default function (content: string, options: HighlightOptions = {lang: 'bash'}) {
    if (options.classPrefix) {
        highlightjs.configure({
            classPrefix: options.classPrefix
        });
    }
    let splitReg = options.splitReg || '';
    let lang = options.lang;
    let eascapeContent = content.trim().replace(/</g, '&lt;').replace(/>/g, '&gt;');
    if (!highlightjs.getLanguage(lang)) {
        return eascapeContent;
    }
    if (splitReg) {
        const lines = eascapeContent.split(splitReg);
        return lines.map(line => {
            return highlightjs.highlight(lang, line, true).value;
        });
    }
    let res = highlightjs.highlight(lang, eascapeContent, true);
    return res.value;
}
